...
static void gwm_rx_hook(const CANPacket_t *msg) {
  if (msg->bus == GWM_MAIN_BUS) {
    ...
    if (msg->addr == GWM_SPEED) {
      ...
      float speed = (float)((fr + rr + rl + fl) / 4.0f * 0.05924739f * GWM_KPH_TO_MS);
...
static bool gwm_tx_hook(const CANPacket_t *msg) {
  const SteeringLimits GWM_STEERING_LIMITS = {
    // SAFETY: kept at 253. Raising to 450 made the GWM EPS reject the steer
    // request in hard curves and release the wheel mid-corner at highway speed
    // (route 00000074--940e632241). Must match CarControllerParams.STEER_MAX.
    .max_steer = 253,
    .max_rate_up = 4,
    .max_rate_down = 6,
    .max_torque_error = 80,
  ...
  if (msg->bus == GWM_MAIN_BUS) {
    if (msg->addr == GWM_STEER_CMD) {
      ...
      violation |= steer_torque_cmd_checks(desired_torque, steer_req, GWM_STEERING_LIMITS);
    }
...
static safety_config gwm_init(uint16_t param) {
  static const CanMsg GWM_TX_MSGS[] = {
    ...
    {GWM_RX_STEER_RELATED, GWM_CAMERA_BUS, 64}, // EPS steering feedback to camera
    {GWM_STEER_CMD, GWM_MAIN_BUS, 64},          // Steering command
  ...
  static const CanMsg GWM_LONG_TX_MSGS[] = {
    ...
    {GWM_RX_STEER_RELATED, GWM_CAMERA_BUS, 64}, // EPS steering feedback to camera
    {GWM_STEER_CMD, GWM_MAIN_BUS, 64},          // Steering command
    {GWM_LONG_CONTROL, GWM_MAIN_BUS, 64},       // Longitudinal control message from camera
    {GWM_HUD, GWM_MAIN_BUS, 64},                // HUD and dashboard
  };
