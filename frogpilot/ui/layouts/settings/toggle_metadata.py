...
GAS_BRAKE_TOGGLES = (
  ...
  ToggleDefinition(
    title=("Target Lateral Acceleration"),
    param="CurveTargetLatAccel",
    description=("<b>The lateral acceleration openpilot aims to hold through curves.</b> The car slows only enough to keep this g-force, so higher values mean faster cornering (less slowing) and lower values mean gentler, slower curves. <b><br><br>Safety:</b> keep this at or below what your steering can physically deliver. On the GWM Haval the EPS tops out around 1.3 m/s²; pushing beyond that saturates the steering and can fault it mid-curve, so the default 1.25 is the recommended sweet spot."),
  ...
  ToggleDefinition(
    ...
    description=("<b>The learned lateral acceleration from collected driving data.</b> Shown for reference; the \"Target Lateral Acceleration\" above is what actually controls curve speed."),
    parent_param="CurveSpeedController",
...
VEHICLE_SETTINGS_TOGGLES = (
  ...
  ToggleDefinition(
    title=("DSU Re-Route Harness"),
    param="ToyotaDSUBypass",
    car_params=["toyota_brand"],
    description=("<b>Enable openpilot longitudinal control using a DSU re-route harness.</b> This harness wires the DSU's ACC messages onto the camera CAN bus so openpilot can send its own longitudinal commands. Only enable this after physically installing the harness on your TSS-P vehicle."),
    parent_param="ToyotaToggles",
    reboot_required=True,
    tuning_level=2,
  ),
